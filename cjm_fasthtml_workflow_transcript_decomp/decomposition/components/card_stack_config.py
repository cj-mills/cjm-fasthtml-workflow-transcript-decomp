"""Card stack configuration constants for the Phase 2 decomposition UI"""

# AUTOGENERATED! DO NOT EDIT! File to edit: ../../../nbs/decomposition/components/card_stack_config.ipynb.

# %% auto #0
__all__ = ['DECOMP_CS_CONFIG', 'DECOMP_CS_IDS', 'DECOMP_CS_BTN_IDS', 'DECOMP_TS_CONFIG', 'DECOMP_TS_IDS']

# %% ../../../nbs/decomposition/components/card_stack_config.ipynb #cs-cfg-constants
from cjm_fasthtml_card_stack.core.config import CardStackConfig, CardStackStyleConfig
from cjm_fasthtml_card_stack.core.html_ids import CardStackHtmlIds
from cjm_fasthtml_card_stack.core.button_ids import CardStackButtonIds

from cjm_fasthtml_token_selector.core.config import TokenSelectorConfig
from cjm_fasthtml_token_selector.core.html_ids import TokenSelectorHtmlIds

DECOMP_CS_CONFIG = CardStackConfig(
    prefix="sd-decomp",
    disable_scroll_in_modes=("split",),
    style=CardStackStyleConfig(
        viewport_padding_x='0.0rem', 
        viewport_padding_y='0.0rem'
    )
)

DECOMP_CS_IDS = CardStackHtmlIds(prefix=DECOMP_CS_CONFIG.prefix)
DECOMP_CS_BTN_IDS = CardStackButtonIds(prefix=DECOMP_CS_CONFIG.prefix)

DECOMP_TS_CONFIG = TokenSelectorConfig(
    prefix="sd-decomp-ts",
    selection_mode="gap",
    initial_delay=200,
    repeat_interval=90,
    throttle_floor=50,
)

DECOMP_TS_IDS = TokenSelectorHtmlIds(prefix=DECOMP_TS_CONFIG.prefix)
