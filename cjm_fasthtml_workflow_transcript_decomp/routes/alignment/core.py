...
def _update_alignment_state(
    ...
    updates = {}
    if vad_chunks is not None: updates["vad_chunks"] = vad_chunks
    if focused_chunk_index is not None: updates["focused_chunk_index"] = focused_chunk_index
    if is_initialized is not None: updates["is_initialized"] = is_initialized
    if history is not None: updates["history"] = history
    if visible_count is not None: updates["visible_count"] = visible_count
    if card_width is not None: updates["card_width"] = card_width
    if media_path is not None: updates["media_path"] = media_path
    if audio_duration is not None: updates["audio_duration"] = audio_duration

    if updates:
        workflow.state_store.update_state(
            workflow.config.workflow_id, session_id,
            {"step_states": {"alignment": updates}}
        )
...
def _update_decomp_segments(
    ...
    workflow.state_store.update_state(
        workflow.config.workflow_id, session_id,
        {"step_states": {"decomposition": {"segments": segment_dicts}}}
    )
